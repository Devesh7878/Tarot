...
function buildTopicRanges(topics, totalDays) {
  const fallbackTopic = { name: "Core role skills", topic: "Core role skills and interview practice" };
  const normalizedTopics = topics.length > 0 ? topics : [fallbackTopic];
  const selectedTopics = normalizedTopics.length <= totalDays
    ? normalizedTopics
    : [
        ...normalizedTopics.slice(0, totalDays - 1),
        {
          name: normalizedTopics.slice(totalDays - 1).map((topic) => topic.name).join(" + "),
          topic: normalizedTopics.slice(totalDays - 1).map((topic) => topic.topic).join(" and "),
        },
      ];
